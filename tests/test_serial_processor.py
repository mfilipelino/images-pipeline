...
class TestSerialProcessor:
    ...
    def test_process_batch_single_item(self):
        ...
        with patch(
            "images_pipeline.processors.serial.process_single_image"
        ) as mock_process:
            expected_result = ProcessingResult(
                source_key="test.jpg", dest_key="output/test.jpg", success=True
            )
    ...
    def test_process_batch_multiple_items(self):
        ...
        with patch(
            ...
            expected_results = [
                ProcessingResult(
                    source_key="test1.jpg", dest_key="output/test1.jpg", success=True
                ),
                ProcessingResult(
                    source_key="test2.jpg", dest_key="output/test2.jpg", success=True
                ),
                ProcessingResult(
                    source_key="test3.jpg",
                    dest_key="output/test3.jpg",
                    success=False,
                    error="Failed",
                ),
            ]
        ...
        with patch(
            ...
            def create_result(s3_client, item, config):
                return ProcessingResult(
                    source_key=item.source_key, dest_key=item.dest_key, success=True
                )
    ...
    def test_process_batch_handles_different_config_options(self):
        ...
        configs = [
            ...
            ProcessingConfig(
                source_bucket="src", dest_bucket="dst", transformation="grayscale"
            ),
    ...
    def test_process_batch_with_mixed_success_failure(self):
        ...
        with patch(
            ...
            expected_results = [
                ...
                ProcessingResult(
                    source_key="failure.jpg", success=False, error="Processing failed"
                ),
            ]
            mock_process.side_effect = expected_results
