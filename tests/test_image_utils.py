...
class TestExtractExifData:
    ...
    @patch('images_pipeline.core.image_utils.TAGS', {256: 'ImageWidth', 257: 'ImageLength', 272: 'Make'})
...
class TestSklearnKmeansQuantize:
    ...
    def test_sklearn_kmeans_quantize_with_mock_dependencies(self):
        ...
        with patch('images_pipeline.core.image_utils.sklearn_kmeans_quantize') as mock_func:
            mock_func.side_effect = ImportError("numpy and scikit-learn are required for sklearn_kmeans_quantize")
            
            with pytest.raises(ImportError, match="numpy and scikit-learn are required"):
                mock_func(Mock())
...
class TestNativeKmeansQuantize:
    ...
    @patch('images_pipeline.core.image_utils.sklearn_kmeans_quantize')
    def test_native_kmeans_quantize_shows_deprecation_warning(self, mock_sklearn_kmeans):
